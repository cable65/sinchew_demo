...
### Fixed
- **Deployment**:
  - Removed `outputFileTracing: false` from `next.config.ts` to resolve TypeScript error (`Property 'outputFileTracing' does not exist in type 'NextConfig'`).
  - The previous `ENOENT` error for `middleware.js.nft.json` should be resolved by the migration to `proxy.ts` in v0.1.11.

## [0.1.11] - 2026-01-02

### Fixed
- **Deployment**:
  - Resolved `Both middleware file ... and proxy file ... are detected` error.
  - Merged logic from `src/middleware.ts` into `src/proxy.ts` (Next.js 16+ convention).
  - Deleted `src/middleware.ts` to remove conflict.
  - `src/proxy.ts` now handles both Dashboard Auth and API Security/Rate Limiting.
...
### Fixed
- **Deployment**:
  - Fixed `Property 'ip' does not exist on type 'NextRequest'` TypeScript error in `middleware.ts` by adding type assertion and fallback to `x-forwarded-for` header.
